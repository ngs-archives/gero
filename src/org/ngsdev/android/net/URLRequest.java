...
public class URLRequest implements HttpUriRequest {
  ...
  public void createHttpRequest(String uri, String methodName)
      throws URISyntaxException {
  ...
  public URI getURI() {
    ...
    if (!this.isPost() && params != null) {
      ...
      try {
        URI = new URI(String.format("%s%s%s", strURI, strURI.contains("?") ? ""
  ...
  public boolean send(AbstractHttpClient client, RequestListener listener) {
  ...
  public boolean send(RequestListener listener) {
    return this.send(new DefaultHttpClient(), listener);
  }
